...
describe("translation queue helpers", () => {
  ...
  it("routes only llm providers through the batch queue", async () => {
    ...
    expect(shouldUseBatchQueue(microsoftProvider)).toBe(false)
    expect(shouldUseBatchQueue(googleProvider)).toBe(false)
  ...
  it("does not normalize cached non-Google translations", async () => {
    ...
    const result = await handler({
      data: {
        text: "hello",
        langConfig: DEFAULT_CONFIG.language,
        providerRef: localProviderRef(microsoftProvider),
        scheduleAt: Date.now(),
        hash: "webpage-hash",
      },
    })
