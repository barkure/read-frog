...
import { isLLMProviderConfig, apiProviderConfigItemSchema } from "@/types/config/provider"
...
const streamTextPayloadSchema = baseStreamPayloadSchema
  .extend({
    ...
    providerConfig: apiProviderConfigItemSchema.optional(),
  })
 ...
 *
 * Non-streaming `generateText` callers — the page summary, subtitle
 * segmentation, and language detection — collapse onto this, which reuses the
 * same part stream and reader as page translation.
 */
export async function generateTextForProviderRef(
  ...
  // Local text generation needs a real LLM: pure translate providers (Google,
  ...
  // pair a model from the current row with providerOptions computed from the
  // snapshot the caller captured — and would fail outright for a row deleted
  // from another tab while the config was already in hand.
