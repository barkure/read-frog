...
export class SegmentationPipeline {
   ...
   * ref, so segmentation neither repeats the provider lookup per block nor
   * sends a translate-only provider on a doomed generation per chunk.
   * Null means AI segmentation cannot run (no provider, or the provider has
   * no model to prompt); chunks then fall back to rule-based optimization
   * until a new session resolves a fresh ref.
   */
