...
export class UniversalVideoAdapter implements SubtitlesProvidersAdapter {
  ...
  private toggleSubtitlesWithSource(enabled: boolean) {
  ...
  private async switchSubtitlesFetcher(next: SubtitlesSource): Promise<void> {
    if (next !== SUBTITLES_SOURCE.NATIVE) {
      return
    }
  ...
  private async startTranslation() {
    try {
      ...
      if (error instanceof ToastSubtitlesError) {
        ...
        // the toast branch has to clear it.
        this.subtitlesScheduler?.setState("idle")
  ...
  private async processTranslatedSubtitles() {
    ...
    // depends on: this is the only path that puts captions on screen for the
    // translated flow, so sequencing it after the provider resolve made the
    // original captions wait on work they never needed — and a slow resolve left
    // the player in "loading" showing nothing at all.
    this.publishSourceTrack(this.sessionProcessedFragments)

    // Resolved once per session so the cache key matches the identity the
    // background will use, rather than re-resolving per fragment. Narrowed once
    // for the whole session: segmentation and the summary are generations, so a
    // translate-only provider keeps the rule-based recut and skips the summary
    // without a doomed prompt attempt per look-ahead window.
    ...
    if (useAiSegmentation) {
      this.segmentationPipeline = new SegmentationPipeline({
        ...
        // The session ref covers segmentation too: both subtitle routes
        // resolve the same capability, and per-block re-resolution would repeat
        // that lookup for every look-ahead window.
