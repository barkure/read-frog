...
/**
 * What a content script may hand the background for one streamed call.
 *
 * Deliberately narrow: the sampling parameters the AI SDK accepts (temperature, topP, penalties,
 * seed, stop sequences) and any thinking/reasoning setting are NOT part of this payload. DeepSeek
 * is the only model-backed provider here and its API defaults are what the extension runs on, so
 * nothing in that family is ever sent.
 */
